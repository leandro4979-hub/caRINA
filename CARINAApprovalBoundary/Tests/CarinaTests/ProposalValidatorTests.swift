...
final class ProposalValidatorTests: XCTestCase {
    ...
    func testRejectsParentTraversalOutsideRepo() throws {
        ...
        XCTAssertThrowsError(try validator.validate(proposal, now: now)) {
            XCTAssertEqual($0 as? ProposalValidationError, .pathOutsideScope(path: "/Users/leandro/Documents/Secrets/key.txt"))
        }
    }
    ...
    func testRejectsMetadataModifyWhenDiffDeletesFile() throws {
        ...
        XCTAssertThrowsError(try validator.validate(proposal, now: now)) {
            XCTAssertEqual(
                ...
                .mutationMismatch(path: "\(repoRoot)/Sources/Config.swift", declared: .modify, observed: .delete)
            )
        }
    }
    ...
    func testRejectsHiddenDeletionOmittedFromMetadata() throws {
        ...
        XCTAssertThrowsError(try validator.validate(proposal, now: now)) {
            XCTAssertEqual(
                $0 as? ProposalValidationError,
                .mutationMismatch(path: "\(repoRoot)/Sources/Unused.swift", declared: nil, observed: .delete)
            )
        }
    }
    ...
    func testRejectsConflictingOperationsForSameCanonicalPath() throws {
        ...
        XCTAssertThrowsError(try validator.validate(proposal, now: now)) {
            XCTAssertEqual($0 as? ProposalValidationError, .conflictingOperations(path: "\(repoRoot)/Sources/Target.swift"))
        }
    }
    ...
    func testFingerprintBindsFilesystemState() throws {
        ...
        let firstBinding = FilesystemStateBinding(
        ...
        let secondBinding = FilesystemStateBinding(
            path: "\(repoRoot)/A.swift",
            role: .source,
            expectation: .existing,
            device: 1,
        ...
        let lhs = try makeValidator(filesystemBindings: [firstBinding]).validate(proposal, now: now)
        let rhs = try makeValidator(filesystemBindings: [secondBinding]).validate(proposal, now: now)
        XCTAssertNotEqual(lhs.canonicalPayloadDigest, rhs.canonicalPayloadDigest)
    }

    func testRejectsExpiredAndReplayedProposal() throws {
        let expired = makeProposal(expiresAt: now.addingTimeInterval(-1), mutations: [], diff: nil)
        ...
        XCTAssertThrowsError(try makeValidator(replayedProposalIDs: [replayed.proposalID]).validate(replayed, now: now)) {
...
private struct StubFilesystemStateBinder: FilesystemStateCapturing {
    ...
    func capture(for mutations: [ValidatedFileMutation]) throws -> [FilesystemStateBinding] { bindings }
